...
def build_client(base_url: str) -> OpenAI:
    return OpenAI(base_url=base_url, api_key=os.environ.get("OPENAI_API_KEY", "not-needed"))


# ---> run_round() > [perform_chat] > client.chat.completions.create(...)
def perform_chat(client: OpenAI, model: str, index: int, vary_prompt: bool) -> RequestResult:
    tools = [
        {
            "type": "function",
            "function": {
    ...
    base_text = "What's the weather like in Boston today?"
    ...
    try:
        ...
        return RequestResult(index=index, latency_s=end - start, error=str(e))


# ---> main() > [run_round] > ThreadPoolExecutor coordinates parallel requests
def run_round(
    client: OpenAI, model: str, num_requests: int, concurrency: int, vary_prompt: bool
) -> Tuple[List[RequestResult], float]:
    results: List[RequestResult] = []
    started_at = time.perf_counter()
    with ThreadPoolExecutor(max_workers=concurrency) as pool:
        futures = [
            pool.submit(perform_chat, client=client, model=model, index=i, vary_prompt=vary_prompt)
...
def summarize_latencies(results: List[RequestResult]) -> dict:
    ...
    summary = {
        ...
        "min_s": round(latencies[0], 4) if latencies else 0.0,
...
def main() -> None:
    ...
    for r in range(1, num_rounds + 1):
        ...
        overall_latencies.extend([res.latency_s for res in results])
